...
1. Fill in the model provider value in \`.env\` if you want to send local test messages. \`.env.example\` documents the adapter values needed later for production chat traffic.
...
${adapterVars.map((name) => `    ${pc.bold(name)}`).join("\n")}
