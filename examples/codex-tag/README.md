...
workspace example and the source copied by `pnpm create heypi codex-tag`.
...
Copy `.env.example` to `.env` if you want to configure Slack, a model, GitHub access, or web search.
...
The example explicitly uses the `heypi-codex-tag:local` Docker image. `pnpm dev` builds it from
`Dockerfile` if it is missing. The image is based on the devcontainer TypeScript/Node image and adds
GitHub CLI plus common repo tools. To let Codex inspect issues, push branches, and open PRs:
