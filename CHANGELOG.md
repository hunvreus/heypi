...
## [0.2.0-beta.0] - 2026-06-23

### Breaking
- Renamed the durable agent instruction surface from `prompt`/`soul` to `instructions`. `loadAgent()` now reads `agent/instructions.md` and optional `agent/system.md`; `agent/AGENTS.md`, `agent/SOUL.md`, and `agent/SYSTEM.md` are no longer convention files.
- Changed webhook `replyUrl` callbacks to require HTTPS by default; HTTP callbacks now require `unsafeReplyHttp: true`.
...
### Added
- Added webhook `replyUrls` and `replyOrigins` callback allowlists for stricter `replyUrl` delivery control.
...
- Added a migration guide for `0.2.0-beta.0` breaking changes.

### Changed
- Changed docs file tree examples to use compact Unicode tree markers.
...
- Changed admin memory from a cramped table to a wrapped list layout with per-file metadata and details.
- Changed admin thread lists to hide local dev test conversations by default, with `admin.localThreads` as the opt-in.
...
### Fixed
- Fixed duplicate card wrappers and section descriptions on the admin approvals, jobs, memory, and configuration pages.
...
- Fixed docs for current scaffolded model ids, xAI model credentials, default built-in tools, admin capabilities, API reference exports, example links, and non-OpenAI credential guidance.
...
- Fixed `create-heypi` scaffold tests to build linked local packages before typechecking generated sample apps.
- Fixed the heypi test suite's internal admin adapter test to use a dynamic admin port instead of the default fixed port.
- Fixed stale custom-adapter and bundled-skill documentation references after the instructions rename.
